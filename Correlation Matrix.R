...
data <- read.csv(text = "Country,RedMeat,WhiteMeat,Eggs,Milk,Fish,Cereals,Starch,Nuts,Fr&Veg
...
# Відкидаємо стовпчик Country для кореляційного аналізу
d <- data[,-1]

# Обчислення кореляційних матриць
pearson_corr <- cor(d, method = "pearson")
spearman_corr <- cor(d, method = "spearman")
kendall_corr <- cor(d, method = "kendall")

# Візуалізація кореляційних матриць
ggcorr(d, method = c("pairwise", "pearson"), label = TRUE, label_alpha = TRUE) + ggtitle("Кореляційна матриця Пірсона")
ggcorr(d, method = c("pairwise", "spearman"), label = TRUE, label_alpha = TRUE) + ggtitle("Кореляційна матриця Спірмена")
ggcorr(d, method = c("pairwise", "kendall"), label = TRUE, label_alpha = TRUE) + ggtitle("Кореляційна матриця Кендалла")
...
loadings(x_pc)
plot(x_pc$loadings)
text(x_pc$loadings,labels=row.names(x_pc$loadings))
biplot(x_pc)

# Однофакторний дисперсійний аналіз
anova_model <- aov(WhiteMeat ~ RedMeat, data = d)
summary(anova_model)
